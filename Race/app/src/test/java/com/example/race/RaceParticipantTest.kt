...
import com.example.race.ui.RaceParticipant
...
class RaceParticipantTest {
    ...
    // runTest c une version de runBlocking pour les test dcp

    @Test
    fun raceParticipant_RaceStarted_ProgressUpdated() = runTest {
        val expectedProgress = 1
        launch { raceParticipant.run() } // simule le debut dune course
        advanceTimeBy(raceParticipant.progressDelayMillis) // avance le temps de delay comme ca on finit direct le test
        runCurrent()
        assertEquals(
            expectedProgress,
            raceParticipant.currentProgress
        ) // on regarde si c bien egal a 1 et ducoup oui
    }
    ...
    fun raceParticipant_RacePaused_ProgressUpdated() = runTest {
        ...
        runCurrent()
        racerJob.cancelAndJoin() // le joueur sarrete
